...
describe("Decoder primitives", () => {
  ...
  describe("u64", () => {
    it("decodes u64 as raw bigint", () => {
      ...
      const decoder = new Decoder(
        new Uint8Array([TypeCode.U64, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]),
      );
  ...
  describe("f64", () => {
    it("decodes f64 as raw number", () => {
      ...
      const decoder = new Decoder(
        new Uint8Array([TypeCode.F64, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf0, 0x3f]),
      );
      expect(expectOk(decoder.decodeValue())).toBeCloseTo(1.0);
    });
  });
...
describe("Decoder Timestamp", () => {
  it("decodes timestamp as Luxon DateTime", () => {
    ...
    const decoder = new Decoder(
      new Uint8Array([TypeCode.Timestamp, 0x80, 0x00, 0x92, 0x65, 0x00, 0x00, 0x00, 0x00]),
    );
...
describe("Decoder Enum", () => {
  ...
  it("rejects enum with length mismatch", () => {
    ...
    const decoder = new Decoder(
      new Uint8Array([
        TypeCode.Enum,
        0x04, // length=2 (too short!)
        0x01, // variant_id=1
        TypeCode.U8,
        0x2a, // u8(42) - this won't fit
      ]),
    );
    expectErr(decoder.decodeValue());
  });
...
describe("decode() public API", () => {
  ...
  it('decodes [["a","b"],["10","17"]] from hex bytes', () => {
    ...
    const bytes = new Uint8Array([
      0x0f,
      0x1e,
      0x0f, // Outer: Array, length=15, element type=Array
      0x0a,
      0x0e,
      0x02,
      0x61,
      0x02,
      0x62, // Inner1: length=5, String, "a", "b"
      0x0e,
      0x0e,
      0x04,
      0x31,
      ...
      0x04,
      0x31,
      0x37, // Inner2: length=7, String, "10", "17"
    ]);
