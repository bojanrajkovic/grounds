...
import { type RelishValue, Null, Bool, U8 } from "@grounds/core";
...
describe("encodeIterable", () => {
  ...
  it("should handle errors from encodeIterableBytes", async () => {
    // Test that encodeIterableBytes properly propagates encode errors
    // Use a simple valid value to ensure the async generator works
    async function* values(): AsyncGenerator<RelishValue> {
      yield Null;
    }

    const chunks: Array<Uint8Array> = [];
    for await (const chunk of encodeIterableBytes(values())) {
      chunks.push(chunk);
    }
